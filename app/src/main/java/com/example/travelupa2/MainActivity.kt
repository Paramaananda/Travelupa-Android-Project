...
import com.example.travelupa2.ui.theme.Travelupa2Theme
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Get imageDao from the singleton database instance
        imageDao = TravelupaApplication.database.imageDao()
...
fun LoginScreen(
    onLoginSuccess: () -> Unit,
    onRegisterClick: () -> Unit // Add this parameter
) {
    var email by remember { mutableStateOf("") }
    var password by remember { mutableStateOf("") }
    var isPasswordVisible by remember { mutableStateOf(false) }
...
fun TempatItemEditable(
    ...
    if (showDeleteConfirmation) {
        AlertDialog(
            ...
            text = { Text("Are you sure you want to delete ${tempat.nama}?") },
            confirmButton = {
                Button(
                    onClick = {
                        firestore.collection("tempat_wisata")
                                ...
                                for (document in documents) {
                                    document.reference.delete()
                                        .addOnSuccessListener {
                                            ...
                                            tempat.gambarUriString?.let { uri ->
                                                val file = File(uri)
                                                if (file.exists()) {
                                                    file.delete()
                                                }
